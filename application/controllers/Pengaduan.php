...
//var_dump($data);
  ...
  public function detail_pengaduan(){
    $data = [
      ...
      'footer' => 'layouts/footer',
      'data_pengaduan' => $this->M_pengaduan->lihat_data("SELECT p.*, w.nama as nama_pengawas, w2.nama as nama_pengawas2, d.no_telp FROM pengaduan p LEFT JOIN data_pengawas w ON p.id_pengawas=w.id_pengawas LEFT JOIN data_pengawas w2 ON p.id_pengawas2=w2.id_pengawas LEFT JOIN data_pengawas w2 ON p.id_pengawas2=w2.id_pengawas JOIN pengadu d on p.id_user=d.id_user  WHERE p.id_pengaduan=".$this->input->get('id'))->result(),
